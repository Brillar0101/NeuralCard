...
**Schematic symbol:** `Switch:SW_SPDT`, emitted by `gen_schematic.py` in `section_power()`.
...
> ⚠️ **Do not renumber SW3's pins.** Pin 2 is the common pole on `VBAT`, pin 3 the closed
...
- NFC NDEF (rewritable): **dual-record message** — record 1 is the vCard as
  ...
  tag's 512 B. Generated by `gen_ndef_dualrecord.py` (private tools repo); flash
  with the ST25 App once boards arrive — no firmware needed. Verify both flows on
